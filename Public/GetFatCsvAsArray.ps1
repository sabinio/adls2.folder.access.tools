Function Get-FatCsvAsArray {
    ...
    If ($PSBoundParameters.ContainsKey('RecurseOnly')) {
        ...
        $myCsv = $myCsv | Where-Object -Property Recurse -eq "False" | Group-Object Container, Folder, Recurse | ForEach-Object { [pscustomobject]@{Container = $_.Name.split(",")[0].Trim(); Folder = $_.Name.split(",")[1].Trim(); Recurse = $_.Name.split(",")[2].Trim(); Items = $_.Group } }
    }
    return $myCsv
}